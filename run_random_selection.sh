...
if [[ ${SUB_TASK} == none ]]; then
  ...
  TOKENIZERS_PARALLELISM=false \
    ...
    --retriever_mode 'random' --do_test --do_train --do_eval --do_eval_bleu --save_last_checkpoints --always_save_model \
    ...
    --data_dir ${WORKDIR}/data  --prefix_tuning 'pass_tuning' --knowledge_usage 'separate' --old_prefix_dir ${WORKDIR}/data_prefix --cache_path ${CACHE_DIR} --res_dir ${RES_DIR} --res_fn ${RES_FN} \
    2>&1 | tee ${LOG}
else
  # CUDA_VISIBLE_DEVICES=0 \
  TOKENIZERS_PARALLELISM=false \
