...
if [[ ${SUB_TASK} == none ]]; then
  OUTPUT_DIR=${MODEL_DIR}/${META_TASK}/${FULL_MODEL_TAG}
  RES_DIR=results/${META_TASK}/${FULL_MODEL_TAG}
  RES_FN=results/${META_TASK}/${FULL_MODEL_TAG}.txt
else
  OUTPUT_DIR=${MODEL_DIR}/${META_TASK}/${SUB_TASK}/${FULL_MODEL_TAG}
...
if [[ ${SUB_TASK} == none ]]; then
  ...
  TOKENIZERS_PARALLELISM=false \
    ...
    --meta_task ${META_TASK} --model_name ${MODEL_NAME} --data_num ${DATA_NUM}  \
